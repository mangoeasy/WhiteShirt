...
class Session 
{
    ...
    const db_host = CONFIG_DB_HOST; 

    //数据库用户名，需要第三方指定自己的用户名
    const db_user = CONFIG_DB_USER;   

    //数据库密码，需要第三方指定自己的库据库密码
    const db_pwd = CONFIG_DB_PASS; 

    //数据库，需要第三方指定数据库
    const db_name = CONFIG_DB_NAME;      

    //数据库表，需要第三方指定数据表
    const db_table = "ext_session"; 
 ...
 */
//define("MAIN_DOMAIN", ".tuding001.com");   
